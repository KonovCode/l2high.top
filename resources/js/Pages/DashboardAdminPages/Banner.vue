        ...
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex flex-col lg:flex-row justify-between">
                <form v-if="banners.open_option" @submit.prevent="sendForm(banners.open_option_id)"
                    class="option_form flex flex-col bg-gray-800 rounded" enctype="multipart/form-data">
                    <h3 class=" text-white text-xl md:text-3xl text-center my-1 md:my-5 font-bold">
                        l2high.top</h3>
                    <div class=" flex flex-col mt-6 md:mt-3 w-5/6 mx-auto">
                        ...
                        <div class="flex mt-1 md:mt-2 justify-between flex-wrap mt-6 md:mt-0">
                            ...
                            <select v-model="imageForm.buy_term"
                                class="h-8 mb-3 md:h-10 mx-auto text-xs md:text-sm rounded" name="buy_term">
                                <option v-for="(term, key) in services" :key="key" :value="term.id">{{term.show_term}}
                                </option>
                            </select>
                            <button type="submit"
                        ...
                        <tbody>
                            <tr v-for="(banner, key) in banners.all" :key="key"
                                ...
                                <td
                                    class="w-full lg:w-auto p-3 text-gray-800 text-center border border-b text-center block lg:table-cell relative lg:static">
                                    <span
                                        class="lg:hidden absolute top-0 left-0 bg-blue-200 px-2 py-1 text-xs font-bold uppercase">Резерв до</span>
                                        {{banner.reserved ? banner.buy_term : 'пусто'}}
                                </td>
                                ...
                                <td
                                    ...
                                    <Link preserve-scroll as="button" :href="route('admin.banner.activate', banner.id)"
                                        style="min-width: 80px" v-if="banner.reserved === 1 && banner.state === 0"
                                        method="POST"
                                        class="rounded bg-green-500 p-1 text-xs font-bold hover:bg-green-600 text-white">
                                    Добавить
                                    </Link>
                                    <Link preserve-scroll as="button" style="min-width: 80px"
                                ...
                                <td
                                    class="w-full my-auto py-2.5 lg:w-auto border border-b flex justify-evenly text-center relative">
                                    ...
                                    <button v-if="banner.reserved === 0" @click="getOption(banner.id)">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            ...
                                            class="w-7 h-7 cursor-pointer rounded bg-blue-400 mr-2 p-1 text-xs font-bold hover:bg-blue-500 text-white">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                                        </svg>
                                    </button>

                                    <Link v-if="banner.reserved === 1" :href="route('admin.banner.reset', banner.id)"
...
export default {
    ...
    setup() {
        const banners = ref({ all: usePage().props.value.banners.data, open_option: false, open_option_id: null });
        ...
        return { banners, services, addImg, getOption, closeOption, sendForm, imageForm }
    }
}
</script>
