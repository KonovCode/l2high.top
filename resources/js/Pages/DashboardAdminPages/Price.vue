                ...
                <tbody>
                    <tr v-for="(service, key) in services.all" :key="key"
                        ...
                        <td style="max-height: 50px"
                            ...
                            <p v-show="services.updateState === false || services.selected_id !== service.id">
                                {{ service.price }}</p>
                            <svg v-show="services.updateState === false || services.selected_id !== service.id"
                                @click="getUpdatePrice(service.id)" xmlns="http://www.w3.org/2000/svg" fill="none"
                            ...
                            <section v-if="services.updateState && services.selected_id === service.id"
                                class="flex justify-center">
                                <input v-model="form.price" :placeholder="service.price" type="number"
                                    class="w-2/4 h-6 text-xs">

                                <Link @click="closeUpdatePrice()" as="button" :href="route('admin.update.price', service.id)" :data="({ price: form.price })"
                                ...
                                <svg @click="closeUpdatePrice()" xmlns="http://www.w3.org/2000/svg" fill="none"
...
export default {
    name: "Price",
    components: { AuthenticateLayout, Head, Link },
    ...
    setup() {
        ...
        return { services, form, getUpdatePrice, closeUpdatePrice };
